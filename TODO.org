...
*** filter log viewing by workflow name
*** sometimes filter log viewing by task name
...
** for apps with different worker node requirements [also an optimization?]: a separate set of nodes for each workflow
* documentation

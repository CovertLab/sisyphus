...
** Gaia.trigger() doesn't start the workflow unless workers are good and ready
...
** pick an easier way to tunnel to kafka than adding to /etc/hosts (Cloud IAP? ifconfig alias? HOSTALIASES? dynamic port forwarding? VPN?)
...
*** auto-launch and shutdown workers, probably mediated by Gaia. Either accept a worker count from the workflow builder for speed and adjustability or react to the queue length to be more automatic.
...
** remove or update Gaia.launch(): There's no ../../script/launch-sisyphus.sh in the pip and the WCM builder now does it differently
...
** create (and document how) a Compute Engine monitor chart for worker node CPU usage
* DONE

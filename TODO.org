...
** sometimes the WCM WF runs Parca then doesn't continue on to run the following tasks
...
*** add the "instance_id" and "zone" labels to gce_instance monitored resources?
*** (jerry) have gaia pass the task name to sisyphus and use it with sisyphus log/tag
**** check that the given log name tag matches the pattern '[-.\w]+'
...
** when the docker app run returns an error code, try not to re-log the same output lines (since that's confusing to read) and set the severity level to error
...
** (jerry) remove kafka-based logging from gaia client, wcm.py, ssh-tunnel.sh, and *maybe* from sisyphus and gaia jars
** store sisyphus id in logs
** pass the workflow name and the task name to sisyphus, then include them in all its log entries to filter on

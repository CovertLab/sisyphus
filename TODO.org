...
** web UI: show a graph of your current wf's steps, click on a step to see its inputs, outputs, log, and which inputs are available; show the workers and what job/task each one is running
